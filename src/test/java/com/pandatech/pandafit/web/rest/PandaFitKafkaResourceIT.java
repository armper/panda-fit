...
class PandaFitKafkaResourceIT {
    ...
    void setup() {
        ...
        PandaFitKafkaResource kafkaResource = new PandaFitKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/panda-fit-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/panda-fit-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
