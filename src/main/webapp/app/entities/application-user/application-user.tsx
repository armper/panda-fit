...
import { getSearchEntities, getEntities } from './application-user.reducer';
            ...
            <tbody>
              {applicationUserList.map((applicationUser, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{applicationUser.ranking}</td>
                  <td>{applicationUser.internalUser ? applicationUser.internalUser.id : ''}</td>
                  <td>
                    {applicationUser.routines
                      ? applicationUser.routines.map((val, j) => (
                          <span key={j}>
                            <Link to={`routine/${val.id}`}>{val.id}</Link>
                            {j === applicationUser.routines.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
